...
export default function Slider() {
  ...
  const slides = [
    {
      type: "landscape",
      url: '../public/assets/famille/PhotosAccueil-003.jpg',
    },
    {
      type: "landscape",
      url: '../public/assets/smashcake/MG_1604.JPG',
    },

    {
      type: "landscape",
      url: '../public/assets/smashcake/MG_1641.JPG',
    },
    {
      type: "portrait",
      url: "../public/assets/bebe/_MG_1107.JPG"
    },
    {
      type: "portrait",
      url: "../public/assets/bebe/_MG_1180.JPG"
    },
    {
      type: "portrait",
      url: "../public/assets/bebe/_MG_1205.JPG"
    },

  ];
  const isMobile = window.innerWidth <= 768;
  const filteredSlides = isMobile
    ? slides.filter((slide) => slide.type === "portrait")
    : slides.filter((slide) => slide.type === "landscape");
          ...
          <ul className="whitespace-no-wrap flex-no-wrap [backface-visibility: hidden] [transform-style: preserve-3d] [touch-action: pan-Y] [will-change: transform] relative flex w-full overflow-hidden p-0">
            {
              filteredSlides.map((slide) => (
                <li>
                  <img
                    src={slide.url}
                    className="w-full max-w-full max-h-full m-auto h-full  rounded-xl bg-center bg-cover duration-500"
                  />
                </li>
              ))
            }
